...
void aes_encrypt_wrap(AES_Context *ctx, const uint8_t *inp, uint8_t *out)
{
	      ...
	      uint8_t *output)
{
	...
	aes_encrypt_wrap(ctx, nonce_hash, tmp); //encrypt nonce
	int blocks = count/16;
		   ...
		   uint8_t *output)
{
	...
	aes_encrypt_wrap(ctx, nonce_ctr, tmp);
				 ...
				 uint8_t *output)
{
	AES_Context ctx;
	aes_setkey(&ctx, key);
	aes_encrypt_wrap(&ctx, challenge, output);
	for (int i = 0; i < 16; i++) {
